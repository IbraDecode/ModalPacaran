...
const HomeScreen = () => {
  ...
  const progress = room?.meta?.targetAmount ? Math.min(1, totalTogether / room.meta.targetAmount) : 0;
  ...
  const header = (
    <div className="flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-2">
        <button className="h-10 w-10 rounded-full border border-[var(--border)] flex items-center justify-center text-lg">🔔</button>
        <Link to="/settings" className="h-10 w-10 rounded-full border border-[var(--border)] flex items-center justify-center text-lg">
          ⚙️
        </Link>
      </div>
    </div>
  ...
  return (
    <MobileShell header={header} footer={<BottomNav />} className="flex flex-col">
      <div className="flex-1 flex flex-col gap-4 min-h-0">
        <GlassCard className="space-y-3 bg-[var(--surface)]">
          ...
          <div className="flex items-center justify-between text-xs text-[var(--text-secondary)]">
            <span>Hari ini {formatCurrency(myToday + partnerToday)}</span>
            <span className="text-amber-400">{partnerToday === 0 ? '⚠ pasangan lo belum setor' : 'Dua-duanya kebaca'}</span>
          </div>
        </GlassCard>
        ...
        <div className="grid grid-cols-4 gap-3">
          <button
            ...
            disabled={pending}
          >
            <span className="h-12 w-12 rounded-full bg-[var(--accent)] text-white flex items-center justify-center shadow">+7K</span>
            <span>Tambah</span>
          </button>
          <button
            className="btn flex flex-col items-center gap-1 rounded-2xl bg-[var(--surface)] border border-[var(--border)] py-3 text-xs text-[var(--text-primary)]"
          ...
          <Link
            ...
            className="btn flex flex-col items-center gap-1 rounded-2xl bg-[var(--surface)] border border-[var(--border)] py-3 text-xs text-[var(--text-primary)]"
          >
            <span className="h-12 w-12 rounded-full bg-[var(--border)] flex items-center justify-center">📜</span>
            <span>History</span>
          </Link>
          <Link
            to="/settings"
            className="btn flex flex-col items-center gap-1 rounded-2xl bg-[var(--surface)] border border-[var(--border)] py-3 text-xs text-[var(--text-primary)]"
          >
            <span className="h-12 w-12 rounded-full bg-[var(--border)] flex items-center justify-center">ℹ️</span>
            <span>Room</span>
          </Link>
        </div>
